...
public class AudioMgr : Singleton<AudioMgr>
{
	...
	{
		ApplyMixerVolume("MasterVol", GlobalVolume);
		ApplyMixerVolume("MusicVol", MusicVolume);
		ApplyMixerVolume("SfxVol", SfxVolume);
	}

	/// <summary>
	///     Apply a linear slider value (0-1) to the mixer in decibels.
	/// </summary>
	...
	/// <summary>
	///     Convert linear slider values (0-1) to decibels for the mixer.
	/// </summary>
	private static float LinearToDecibels(float linearVolume)
	{
		if (linearVolume <= 0f) return MinDecibels;
		return Mathf.Log10(Mathf.Clamp(linearVolume, 0.0001f, 1f)) * 20f;
	}

	/// <summary>
	///     Update master volume from a linear slider value (0-1).
	/// </summary>
	public void SetMasterVolume(float linearVolume, bool saveValue = true)
	{
		if (saveValue) GlobalVolume = linearVolume;
		ApplyMixerVolume("MasterVol", linearVolume);
	}

	/// <summary>
	///     Update music volume from a linear slider value (0-1).
	/// </summary>
	public void SetMusicVolume(float linearVolume, bool saveValue = true)
	{
		if (saveValue) MusicVolume = linearVolume;
		ApplyMixerVolume("MusicVol", linearVolume);
	}

	/// <summary>
	///     Update sfx volume from a linear slider value (0-1).
	/// </summary>
	public void SetSfxVolume(float linearVolume, bool saveValue = true)
	{
		if (saveValue) SfxVolume = linearVolume;
		ApplyMixerVolume("SfxVol", linearVolume);
	}

...
namespace FIMSpace
{
	...
	public class LeaningAnimator : FimpossibleComponent
	{
		...
		private LeaningProcessor Leaning = new();

		private float _sd_origF;

		private float _sd_origS;

		public LeaningProcessor Parameters => Leaning;

		/// <summary> Manually inform leaning that character is accelerating (turn off "TryAutoDetectAcceleration" fist) </summary>
		public bool SetIsAccelerating
		{
			set => Parameters.IsCharacterAccelerating = value;
		}

		/// <summary> Manually inform leaning that character is on the ground </summary>
		public bool SetIsGrounded
		{
			set => Parameters.IsCharacterGrounded = value;
		}

		public bool CheckIfIsGrounded => Parameters.IsCharacterGrounded;
		public bool CheckIfIsAcccelerating => Parameters.accelerating;
		...
		private void OnEnable()
		{
			...
			if (Leaning == null) Leaning = new LeaningProcessor();
			AssemblyReloadEvents.afterAssemblyReload += () => Leaning.Initialize(this);
#endif
#endif
		}
		...
		{
			var targetV = Mathf.SmoothDamp(
				Leaning._UserOriginBoneAddForwardAngle, forwardAngle, ref _sd_origF, duration, 1000000f, deltaTime);
		...
		{
			var targetV = Mathf.SmoothDamp(
				Leaning._UserOriginBoneAddSideAngle, sideAngle, ref _sd_origS, duration, 1000000f, deltaTime);

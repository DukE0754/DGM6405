...
namespace FIMSpace
{
    public partial class LeaningProcessor
    {
        ...
        public static void Editor_DrawTweakFullGUI( SerializedProperty sp_LeaningProcessor, ref LeaningAnimator.ELeaningEditorCat drawSetup, LeaningAnimator get = null )
        {
            ...
            if( drawSetup == LeaningAnimator.ELeaningEditorCat.Setup ) GUI.backgroundColor = Color.green;

            if( GUILayout.Button( new GUIContent( drawSetup == LeaningAnimator.ELeaningEditorCat.Setup ? " Setup" : "", FGUI_Resources.Tex_GearSetup, "Show Setup Category" ), FGUI_Resources.ButtonStyle, GUILayout.Height( 24 ) ) )
            {
                drawSetup = LeaningAnimator.ELeaningEditorCat.Setup;
            }

            if( drawSetup == LeaningAnimator.ELeaningEditorCat.Leaning ) GUI.backgroundColor = Color.green; else GUI.backgroundColor = bg;

            if( GUILayout.Button( new GUIContent( drawSetup == LeaningAnimator.ELeaningEditorCat.Leaning ? " Tweak" : "", FGUI_Resources.Tex_Tweaks, "Open Leaning Animation Settings Category" ), FGUI_Resources.ButtonStyle, GUILayout.Height( 24 ) ) )
            {
                drawSetup = LeaningAnimator.ELeaningEditorCat.Leaning;
            }
            ...
            if( drawSetup == LeaningAnimator.ELeaningEditorCat.Setup )
            {
                ...
                var sp_feet = sp_LeaningProcessor.FindPropertyRelative( "FootsOrigin" );
                if( sp_feet.objectReferenceValue == null ) GUI.backgroundColor = new Color( 1f, 1f, 0.65f );
                EditorGUILayout.PropertyField( sp_feet, new GUIContent( "Feet Origin", sp_feet.tooltip ) );

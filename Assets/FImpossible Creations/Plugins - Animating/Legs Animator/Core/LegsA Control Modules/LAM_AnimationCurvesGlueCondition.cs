...
namespace FIMSpace.FProceduralAnimation
{
	...
	{
		private Variable _play_AllowHeightGlueOnLevels;
		private Variable _play_FloorValueBelow;
		private Variable _play_IgnoreMidConditions;
		private LegsAnimatorCustomModuleHelper _useHelper;

		private List<int> animatorHashes;
		private bool initialized;
		private Variable FloorValueBelowVar => _useHelper.RequestVariable("Floor Value Below", 0.01f);
		private Variable IgnoreMidConditionsVar => _useHelper.RequestVariable("Ignore Mid Conditions", false);
		private Variable AllowHeightGlueOnLevelVar => _useHelper.RequestVariable("Allow Height Glue On Level", -1f);
		...
		public override void Leg_LateUpdate(LegsAnimatorCustomModuleHelper helper, Leg leg)
		{
			...
			var value = LA.Mecanim.GetFloat(animatorHashes[leg.PlaymodeIndex]);
			...
			if (value <= _play_FloorValueBelow.GetFloat()) // Value below ground level - GROUNDED
			{
				...
				if (_play_IgnoreMidConditions.GetBool()) leg.G_CustomForceNOTDetach = true;
...
#region Editor Code
		...
		public override void Editor_InspectorGUI(LegsAnimator legsAnimator, LegsAnimatorCustomModuleHelper helper)
		{
			...
			{
				EditorGUILayout.HelpBox("No Animator found to handle animation curves!", MessageType.Warning);
				return;
			}

			EditorGUILayout.HelpBox(
				"Using animation curve parameters to controll gluing timing. It requires extra curves inside animation clips but can provide better controll for gluing feature.",
				MessageType.Info);
			GUILayout.Space(5);
			...
			if (!ignMidV.TooltipAssigned)
				ignMidV.AssignTooltip(
					"When enabled algorithm will not check detaching conditions on foot rotation angles");
			...
			var targetCount = legsAnimator.Legs.Count;
			...
			for (var l = 0; l < animatorHashes.Count; l++)
			{
				EditorGUILayout.BeginHorizontal();
				var val = LA.Mecanim.GetFloat(animatorHashes[l]);
				EditorGUILayout.LabelField("  [" + l + "] " + val, GUILayout.Width(106));
				EditorGUILayout.LabelField(LA.Legs[l].Side.ToString(), GUILayout.Width(54));
				EditorGUILayout.LabelField(
					"  " + (val < _play_FloorValueBelow.GetFloat() ? "FOOT GROUNDED" : "FOOT MOVING"));

...
{
	[CreateAssetMenu(
		fileName = "LAM_AnimCurveGlueCondition_Above",
		menuName = "FImpossible Creations/Legs Animator/Control Module - Animation Curves Glue Condition Above",
		order = 5)]
	public class LAM_AnimationCurvesGlueCondition_Above : LegsAnimatorControlModuleBase
	{
		...
		private Variable _play_FloorValueAbove;
		...
		private Variable FloorValueAboveVar => _useHelper.RequestVariable("Floor Value Above", 0.99f);
		...
		public override void Leg_LateUpdate(LegsAnimatorCustomModuleHelper helper, Leg leg)
		{
			...
			else
			{
				...
				if (_play_IgnoreMidConditions.GetBool()) leg.G_CustomForceDetach = true;
			}
		}
		...
		public override void Editor_InspectorGUI(LegsAnimator legsAnimator, LegsAnimatorCustomModuleHelper helper)
		{
			...
			for (var l = 0; l < animatorHashes.Count; l++)
			{
				...
				EditorGUILayout.LabelField(
					val >= _play_FloorValueAbove.GetFloat() ? "FOOT GROUNDED" : "FOOT MOVING");

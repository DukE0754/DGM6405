...
namespace FIMSpace.AnimationTools
{
    ...
    public partial class AnimationDesignerWindow : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            EditorPrefs.SetFloat("FimpAutoSaveTime", 0);
        }
        ...
        bool moveSecondaryAnimator = false;
        Vector3 seconadaryAnimatorPosition = Vector3.zero;
        Vector3 seconadaryAnimatorRotation = Vector3.zero;
        ...
        private void OnGUI()
        {
            ...
            #region Check for auto-save

            bool isAutoSave = EditorPrefs.GetInt("FimpAutoSave", 0) == 1;
            if (isAutoSave)
            {
                float lastAutoSaveIn = EditorPrefs.GetFloat("FimpAutoSaveTime", 0);
                float currentSaveTime = (float)EditorApplication.timeSinceStartup;
                if ( currentSaveTime - lastAutoSaveIn > 180f)
                {
                    AssetDatabase.SaveAssets();
                    EditorPrefs.SetFloat("FimpAutoSaveTime", currentSaveTime);

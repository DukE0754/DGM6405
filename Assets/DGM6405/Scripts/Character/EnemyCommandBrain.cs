...
public class EnemyCommandBrain : PausableBehaviour, ICharacterBrain
{
    ...
    private void UpdateAIDecisions()
    {
        ...
        if (_targetTransform != null)
        {
            ...
            if (_aimSystem != null)
            {
                _aimSystem.SetAimTarget(_targetTransform.position);
            }
        }
        else
        {
    ...
    private void ProcessMovementCommands()
    {
        ...
        if (_movementSystem != null)
        {
            _movementSystem.ApplyMovement(_aiMoveInput, _aiSprint);
        }

        // Process jump and gravity
        if (_jumpGravitySystem != null)
        {
            _jumpGravitySystem.TickVertical(_aiJump);
        }
    }
    ...
    private void ProcessCombatCommands()
    {
        ...
        if (_blockSystem != null)
        {
            _blockSystem.SetBlocking(_aiBlock);
        }

        // Process shoot
        if (_shootSystem != null)
        {
            _shootSystem.TryShoot(_aiShoot);
        }

        // Process melee
        if (_meleeSystem != null)
        {
            _meleeSystem.TryMelee(_aiMelee);
        }
    }
